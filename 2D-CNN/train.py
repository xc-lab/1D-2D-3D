...
from models import AlexNet, FCN, Mnist
...
def main():
    ...
    parser.add_argument('--epochs', metavar='N', type=int, default=60,

                        help='number of total epochs to run')

    parser.add_argument('--start-epoch', metavar='N', type=int, default=0,
                        help='manual epoch number (useful on restarts)')
    parser.add_argument('-b', '--batch-size', metavar='N', type=int, default=8,
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)

    val_dataset = MetaDataset(root=args.data, train=False, transform=transform)
    # val_dataloader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False)
    val_dataloader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True, drop_last=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, data in enumerate(train_bar):
            ...
            if not args.large_batch_size:
                ...
                train_bar.set_description(desc='Epoch[%003d/%d]: Training, loss: %.4f, acc: %.4f' % (epoch, args.epochs,
                ...
                if (i + 1) % accumulation_steps == 0:  # Wait for several backward steps
                    ...
                    train_bar.set_description(desc='Epoch[%003d/%d]: Train, loss = %.4f, acc = %.4f' % (epoch + 1, args.epochs,
                                                                                  running_results['loss'] /
                                                                                  running_results['total_data_size'],
                                                                                  running_results['acc'] /
                                                                                  running_results['total_data_size']))
        ...
        with torch.no_grad():
            ...
            if val_epoch_loss < min_loss:
                ...
                print('better epoch: {}, better_acc: {:.2f}, min_loss: {:.2f}'.format(best_epoch, best_acc, min_loss))
