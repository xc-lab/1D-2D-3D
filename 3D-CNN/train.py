...
from models import Mnist, FCN, AlexNet


def main():
    parser = argparse.ArgumentParser(description='PyTorch Training: Parkinson diagnose')
    parser.add_argument('--net-model', metavar='N', type=str, default='AlexNet',
                        help='classification model', dest='model', choices=['Mnist', 'AlexNet', 'DNet'])

    parser.add_argument('--data', metavar='DIR', type=str, default='./data/training_data/',
    ...
    parser.add_argument('-b', '--batch-size', metavar='N', type=int, default=4,
                        help='mini-batch size (default: 256)')
    parser.add_argument('--weighted-loss', type=bool, default=False,
                        help='Path of checkpoints')
    parser.add_argument('-j', '--workers', metavar='N', type=int, default=16,
                        help='number of data loading workers (default: 4)')
    # parser.add_argument('--outputs', metavar='RES', type=str, default='./checkpoints/best_results',
    #                     help='Path of output')
    ...
    parser.add_argument('--opt', default='RMSprop', choices=['Adam', 'SGD', 'RMSprop'],
                        help='loss: ' + ' | '.join(['Adam', 'SGD']) + ' (default: Adam)')

    parser.add_argument('--lr', '--learning_rate', metavar='LR', type=float, default=1e-5,
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=8, pin_memory=True, drop_last=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, data in enumerate(train_bar):
            ...
            if not args.large_batch_size:
                ...
                train_bar.set_description(desc='Epoch[%003d/%d]: Training, loss: %.4f, acc: %.4f, lr: %.4f' % (epoch + 1, args.epochs,
                ...
                if (i + 1) % accumulation_steps == 0:  # Wait for several backward steps
                    ...
                    train_bar.set_description(desc='Epoch[%003d/%d]: Train, loss = %.4f, acc = %.4f, lr: %.6f' % (epoch + 1, args.epochs,
                                                                                  running_results['loss'] /
                                                                                  running_results['total_data_size'],
                                                                                  running_results['acc'] /
                                                                                  running_results['total_data_size'],
        ...
        with torch.no_grad():
            ...
            for j, data in enumerate(val_bar):
                ...
                # pred = torch.sum(pred_labels == targets.data)
                # acc = pred / args.batch_size
                ...
                val_bar.set_description(desc='                Test, loss = %.4f, acc = %.4f, lr=%.6f' % (
            ...
            if val_epoch_acc > best_acc and val_epoch_loss < min_loss:
                best_epoch = epoch + 1
                ...
                }, os.path.join(best_dir, 'model_' + args.model + '_best_X256.pth.tar'))

                print('best epoch: {}, best_acc: {:.2f}, min_loss: {:.2f}'.format(best_epoch, best_acc, min_loss))

        # 检查您的pytorch版本如果是V1.1.0+，那么需要将scheduler.step()在optimizer.step()之后调用
        # scheduler.step(val_epoch_loss)

    print('\nbest epoch: {}, best_acc: {:.2f}, min_loss: {:.2f}'.format(best_epoch, best_acc, min_loss))
