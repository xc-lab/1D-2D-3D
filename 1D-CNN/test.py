...
from dataset import DatasetList, FCNDatasetList
...
def main():
    ...
    parser.add_argument('-w', '--weights', metavar='W', type=str, default='./checkpoints/alexnet/2023_05_03_13_17_51/model_AlexNet_best_X128.pth.tar',
    ...
    elif args.model == 'alexnet' or 'AlexNet' :
        model = AlexNet()
    elif args.model == 'FCN' :
        model = FCN()
    else:
    ...
    test_dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False, num_workers=2, pin_memory=True,
                                 drop_last=True)
    ...
    mcc = metric.mcc()
    print(
        "Method1 (On Sequence): Accuracy(ACC) = {:f}, F1_score = {:f}, Recall(Sensitivity,TPR) = {:f}, and Precision(PPV) = {:f}, and Specificity(TNR) = {:f}, and Matthews correlation coefficient(MCC) = {:f}. \n".format(
            acc_score, f1_score, recall_score, precision_score, specificity, mcc))
